...
const Main = ({ setError, setErrMsg }) => {
  const { blockChainData, account, updateRewardBalance } = useContext(
    GlobalContext
  );
      ...
      <div className="main flex items-center justify-between flex-wrap md:flex-nowrap">
        <div className="w-full md:w-1/2 md:pr-8 mb-8 z-10">
          <div className="stakePanel bg-[color:var(--cards-bg)] p-6 w-full">
            <h3 className="uppercase font-semibold text-md font-Poppins text-left">
              Participate Meta - Quantum Stake
            </h3>
            <h2 className="font-extrabold text-2xl ml-3 text-left">
            ...
            <div className="flex min-h-[8rem] bg-[color:var(--cards-bg)] w-full p-8 items-start justify-between text-left">
              ...
              <div>
                ...
                >
                  ...
                  <path
                    d="  M0,30  h8  v-0.7098142461564727  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M10,30  h8  v-3.8886121736656065  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M20,30  h8  v-7.535516905963142  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M30,30  h8  v-9.867032085607057  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M40,30  h8  v-14.308029708931354  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M50,30  h8  v-13.650666700270385  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M60,30  h8  v-17.03132200231409  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                  <path
                    d="  M70,30  h8  v-19.35071883329862  q0,-0 -0,-0  h-8  q-0,0 -0,0  Z"
                    fill="url(#linear-gradient)"
                  />
                </svg>
              </div>
        ...
        <div className="stakePanel bg-[color:var(--cards-bg)] p-6 w-full mb-3">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mt-4 md:space-x-10 truncate">
            <div className="bg-slate-200 bg-opacity-5 px-6 py-4 w-full rounded-lg mb-6 md:mb-0">
              <h2 className="text-xl text-center font-bold mb-4 uppercase border-b pb-2 border-white border-opacity-20">
                6 Months
              </h2>
              <p className="text-sm">
                Rewards Earned:
                {blockChainData.RewardBalance
                  ? parseFloat(blockChainData.RewardBalance).toFixed(2)
                  : "0.00"}
                {CONFIG.tokenSymbol}
              </p>
              <p className="text-sm">
                Stake Amount:
                {parseFloat(blockChainData.StakedBalance).toFixed(2)}
                {CONFIG.tokenSymbol}
              </p>
            </div>
            <div className="bg-slate-200 bg-opacity-5 px-6 py-4 w-full rounded-lg mb-6 md:mb-0">
              ...
              <p className="text-sm">
                Rewards Earned:
                {blockChainData.RewardBalance1
                  ? parseFloat(blockChainData.RewardBalance1).toFixed(2)
                  : "0.00"}
                {CONFIG.tokenSymbol}
              </p>
              <p className="text-sm">
                Stake Amount:
                {parseFloat(blockChainData.StakedBalance1).toFixed(2)}
                {CONFIG.tokenSymbol}
              </p>
            </div>
            <div className="bg-slate-200 bg-opacity-5 px-6 py-4 w-full rounded-lg mb-6 md:mb-0">
              ...
              <p className="text-sm">
                Rewards Earned:
                {blockChainData.RewardBalance2
                  ? parseFloat(blockChainData.RewardBalance2).toFixed(2)
                  : "0.00"}
                {CONFIG.tokenSymbol}
              </p>
              <p className="text-sm">
                Stake Amount:
                {parseFloat(blockChainData.StakedBalance2).toFixed(2)}
                {CONFIG.tokenSymbol}
              </p>
            </div>
          </div>
