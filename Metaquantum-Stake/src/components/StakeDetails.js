...
const StakeDetails = ({ plan, plans, setPlan }) => {
  ...
  useEffect(() => {
    ...
    blockChainData.sixMonthApy,
    blockChainData.oneYearApy,
    blockChainData.threeYearApy,
  ]);
      ...
      <div className="p-6 flex items-center justify-center space-x-2 md:space-x-8">
        <button
          className={classNames(
            "uppercase min-w-[90px] bg-purple-900 rounded-full truncate text-sm px-2 py-1 border border-white hover:text-purple-400",
            {
              "border-4 font-bold": plan.plan === 0,
            }
          )}
        ...
        <button
          className={classNames(
            "uppercase min-w-[90px] bg-purple-900 rounded-full truncate text-sm px-2 py-1 border border-white hover:text-purple-400",
            {
              "border-4 font-bold": plan.plan === 1,
            }
          )}
        ...
        <button
          className={classNames(
            "uppercase min-w-[90px] bg-purple-900 rounded-full truncate text-sm px-2 py-1 border border-white hover:text-purple-400",
            {
              "border-4 font-bold": plan.plan === 2,
            }
          )}
